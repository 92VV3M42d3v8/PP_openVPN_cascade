...
function check_inactivity {
	...
	then
		{
			echo -e "\n----------ATTENTION----------"
			echo -e "It is now $(date)"
			echo -e "At least one server in the cascade is no longer accessible!"
			echo -e "Restart services now, so that a safe state can be restored!"
		} >> $logfile_watchdog
		...
		then
			{
				echo -e "\n----------Attention----------"
				echo -e "It is now $(date)"
				echo -e "Public IP has changed:\t$(wget -O - -q --tries=3 --timeout=20 ipv4.icanhazip.com)"
				echo -e "Restart services now, so that a safe state can be restored!"
			} >> $logfile_watchdog
			kill_primary_process
...
do
	...
	case "$chkfl" in
		...
		0)
			...
			# if after a certain counter the file does not exist yet, something is wrong
			# in this case restart the primary process
			if [ "$timeout" -eq "10" ]
			then
				kill_primary_process
			fi
