...
function vpn_connect_following_n {
	echo -e "\nVPN-Previous Nr. $hopnr is built up according to:\t\t\t$server_name" >> $logfile_script
	...
	do
		...
		if (( $(echo "$errorcount > $inc_timeout" | bc -l) ));
		then
			echo -e "TIMEOUT: Connection to the $hopnr. HOP Server: $server_name NOT successful, try again!" >> $logfile_script
			return=1
			return
		fi
		errorcount=$(echo "$errorcount+0.2" | bc)
...
then
	{
		...
		echo -e "Number of Configs in the array:\t$server_list_count".
		echo "MaxHOP must be less than or equal to number of Configs!!!
		echo "Script is now restarted continuously until the number of Configs is equal or less! Please adjust and restart the service!
...
do
	...
	{
		echo -e "\n\n------------------------------------------------------------------"
		echo "The following connection will remain for $timer seconds"
		echo "------------------------------------------------------------------"
	} >> $logfile_script
	...
	do
		...
		# check, a connection exists / the source server is used
		if wget -O - -q --tries=3 --timeout=20 ipv4.icanhazip.com | grep "$current_state" >> /dev/null
		then
			# 10 Wait seconds before checking again
			sleep 10
			get_cur_tim
		else
			...
			then
				...
				# determine the first server and consolidate the array
				remux_server_list
				...
				do
					...
					then
						...
						write_timestamp
						echo -e "\nThere are no functional servers left over!\n" >> $logfile_script
						echo -e "Now start completely from scratch!" >> $logfile_script
						exit 1
					fi
				done
				# Initial connection established

				# Save the server name of the connection in an array
				con_servers[$k]=$server_name
				k=$((k+1))
				hopnr=$((hopnr+1))

				# should now further connections be established?
				# If maxhop > 1, then go!
				if [ "$maxhop" -gt "1" ];
				then
				...
				echo -e "Connection procedure:\t$endtim_dat" >> $logfile_script

				# now we are finally connected and set a flag
				connected_check=1
			else
				...
				echo -e "-------------------" >> $logfile_script
				write_timestamp
				echo -e "\nWait for watchdog service until processes are restarted!" >> $logfile_script
				sleep 20

				# ATTENTION: if the watchdog does not run, just stop it from here
				exit 1
			fi
		fi
	done
	# out of the loop, because the countdown has expired, now remove everything and then go back to the beginning of the loop (outer loop)
	### END inner loop ###

	# tell the watchdog to wait again until the next connect
	echo "Wait" > $checkfile_watchdog

	echo "Time out! The connections are now terminated!" >> $logfile_script
